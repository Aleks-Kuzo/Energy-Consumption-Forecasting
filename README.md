...
Модель оценивается по трём метрикам на тестовой выборке:

| Метрика | Описание |
|---|---|
| MAE | Средняя абсолютная ошибка, МВт |
| RMSE | Корень из среднеквадратичной ошибки, МВт |
| MAPE | Средняя абсолютная процентная ошибка, % |

*(Замените на фактические цифры после запуска ноутбука на реальном датасете.)*

Пример графика — сопоставление факта и прогноза за первые 200 часов тестового периода:

![prediction plot](assets/prediction_example.png)
